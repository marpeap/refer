...
export default function Home() {
  return (
    <main style={{ minHeight: '100vh', background: '#0A0F1C', color: '#fff', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-outline:hover { background: rgba(59,130,246,0.08) !important; }
      `}</style>

      {/* Background circles */}
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-20%', left: '60%', width: 600, height: 600, background: 'radial-gradient(circle, rgba(59,130,246,0.08) 0%, transparent 70%)', borderRadius: '50%' }} />
        <div style={{ position: 'absolute', bottom: '-10%', left: '-10%', width: 500, height: 500, background: 'radial-gradient(circle, rgba(139,92,246,0.06) 0%, transparent 70%)', borderRadius: '50%' }} />
      </div>
      ...
      <nav style={{ position: 'relative', zIndex: 10, borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '0 40px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', backdropFilter: 'blur(12px)' }}>
        <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 18, fontWeight: 800 }}>
          ...
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', fontFamily: "'Inter', sans-serif", fontWeight: 400, marginLeft: 8, background: 'rgba(59,130,246,0.12)', padding: '2px 7px', borderRadius: 100, border: '1px solid rgba(59,130,246,0.2)' }}>Apporteurs</span>
        </span>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <Link href="/register" className="btn-primary" style={{ padding: '8px 18px', background: '#3B82F6', color: '#fff', textDecoration: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700 }}>
            Devenir apporteur
          </Link>
      ...
      <section style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '80px 24px 64px', maxWidth: 800, margin: '0 auto' }}>
        <div style={{ display: 'inline-block', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 100, padding: '5px 16px', fontSize: 12, color: '#93C5FD', fontWeight: 600, letterSpacing: '0.04em', marginBottom: 24 }}>
          PROGRAMME APPORTEURS D'AFFAIRES
        </div>
        <h1 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 'clamp(36px, 6vw, 60px)', fontWeight: 800, lineHeight: 1.1, marginBottom: 24 }}>
          Recommandez Marpeap.<br />
          <span style={{ color: '#3B82F6' }}>Touchez une commission.</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/register" className="btn-primary" style={{ padding: '14px 32px', background: '#3B82F6', color: '#fff', textDecoration: 'none', borderRadius: 10, fontWeight: 700, fontSize: 16 }}>
      ...
      <section style={{ position: 'relative', zIndex: 1, maxWidth: 900, margin: '0 auto', padding: '0 24px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 28, fontWeight: 800, marginBottom: 8 }}>Comment ça marche ?</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
          {STEPS.map((step, i) => (
            <div key={step.n} style={{
              ...
              <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 42, fontWeight: 800, color: 'rgba(59,130,246,0.15)', lineHeight: 1, marginBottom: 16 }}>{step.n}</div>
              <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 16, fontWeight: 700, marginBottom: 8 }}>{step.title}</div>
              <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6 }}>{step.desc}</div>
              {i < STEPS.length - 1 && (
                <div style={{ position: 'absolute', right: -20, top: '50%', transform: 'translateY(-50%)', color: '#3B82F6', fontSize: 20, display: 'none' }}>→</div>
              )}
            </div>
          ))}
        </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, borderTop: '1px solid rgba(255,255,255,0.05)', padding: '64px 24px 80px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 28, fontWeight: 800, marginBottom: 8 }}>7 solutions à vendre</div>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 32 }}>
            {PACKS_TEASER.map(pack => (
              <div key={pack.name} style={{
                ...
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 14, fontWeight: 800, color: pack.color, marginBottom: 4 }}>{pack.name}</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 6 }}>{pack.desc}</div>
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 18, fontWeight: 800 }}>{pack.price}</div>
              </div>
            ))}
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <Link href="/register" className="btn-primary" style={{ display: 'inline-block', padding: '13px 28px', background: '#3B82F6', color: '#fff', textDecoration: 'none', borderRadius: 10, fontWeight: 700, fontSize: 15 }}>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '24px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'rgba(255,255,255,0.2)', fontSize: 13, flexWrap: 'wrap', gap: 8 }}>
        <span style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800 }}>mar<span style={{ color: '#3B82F6' }}>peap</span></span>
